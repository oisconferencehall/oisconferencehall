...
export function formatSeatDisplay(raw, index) {
  ...
  // 1. Try canonical resolution from actual raw value FIRST
  const canonical = normalizeSeatId(raw);
  if (canonical) {
  ...
  // 2. Handle direct seat number strings e.g. "Seat #17" or "17"
  if (clean.startsWith('Seat #') || clean.startsWith('Seat ') || (!isNaN(parseInt(clean, 10)) && Number(clean) > 0 && Number(clean) <= TOTAL_SEATS)) {
    const num = parseInt(clean.replace(/Seat\s*#?/i, '').trim(), 10);
    if (!isNaN(num)) {
      const entry = Object.entries(SEAT_NUMBERS).find(([, v]) => v === num);
...
export function buildTakenSeatsSet(registrations, eventId) {
  ...
  registrations.forEach(r => {
    ...
    const isForThisEvent = !eventId || !shortId || 
      (r.ticket_id && String(r.ticket_id).toLowerCase().includes(shortId.toLowerCase())) ||
      (r.event_id && String(r.event_id).toLowerCase().includes(shortId.toLowerCase())) ||
      (rawSeat && String(rawSeat).toLowerCase().includes(shortId.toLowerCase())) ||
      (r.ticket_id && String(r.ticket_id).toLowerCase().includes(String(eventId).toLowerCase())) ||
      (rawSeat && String(rawSeat).toLowerCase().includes(String(eventId).toLowerCase())) ||
      !rawSeat.includes('::'); // If seat is stored without event prefix, it applies to current single event
