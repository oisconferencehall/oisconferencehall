...
export function normalizeSeatId(raw) {
  ...
  // Handle "Seat #17" format
  if (clean.startsWith('Seat #')) {
    const num = parseInt(clean.replace('Seat #', '').trim(), 10);
 ...
 */
export function formatSeatDisplay(raw) {
  if (!raw) return 'Reserved Pass';
  const clean = String(raw).split('::').pop().trim();
  if (!clean || clean === 'Reserved Pass' || clean === 'Reserved' || clean === 'General Entry') {
  ...
  if (clean.includes(',')) {
    return clean.split(',').map(s => formatSeatDisplay(s.trim())).join(', ');
  }
  
  const canonical = normalizeSeatId(raw);
  if (!canonical) return clean; // fallback to raw
  const num = SEAT_NUMBERS[canonical];
  if (num === undefined) return clean;
...
export function buildTakenSeatsSet(registrations, eventId) {
  ...
  registrations.forEach(r => {
    ...
    const cleanSeat = rawSeat ? String(rawSeat).split('::').pop().trim() : '';
